...
export async function GET(request: NextRequest) {
  try {
    ...
    if (sessionId) {
      ...
      if (!process.env.STRIPE_SECRET_KEY || '1234') {
      ...
      try {
        ...
        let voucher = await Voucher.findOne({
          ...
          .populate('giftItemId')
          .populate('giftItemId.merchantId');

        if (voucher) {
          console.log(
            '[Checkout Success] Voucher already exists:',
            voucher._id,
          );
          ...
          if (!voucher.emailSent) {
            try {
              ...
              await voucher.save();
              console.log(
                '[Checkout Success] Payment success email sent to existing voucher',
              );
            } catch (emailError) {
              console.error(
                '[Checkout Success] Error sending email to existing voucher:',
                emailError,
              );
            }
          }
        ...
        voucher = await waitForVoucherWithRetry(
        ...
        if (!voucher) {
          console.log(
            '[Checkout Success] Voucher not found, creating from session data (webhook may not have processed yet)',
          );

          const { giftItemId, recipientName, senderName } =
            session.metadata || {};

          if (!giftItemId) {
            console.error(
              '[Checkout Success] Missing gift item information in session metadata:',
              session.metadata,
            );
              ...
              return NextResponse.json(
                {
                  ...
                  details:
                    'This test session was created without the required gift item information. Please create a new test purchase through the application.',
                  suggestion:
                    'Go to the homepage, select a gift item, and complete a new test purchase.',
                  sessionId: sessionId,
                  metadata: session.metadata,
                },
                { status: 400 },
          ...
          if (!giftItem) {
            console.error(
              `[Checkout Success] Gift item not found: ${giftItemId}`,
            );
            return NextResponse.json(
          ...
          try {
            console.log(
              `[Checkout Success] Creating voucher for payment intent: ${session.payment_intent}`,
            );
            ...
            console.log(
              '[Checkout Success] Voucher created/found atomically:',
              {
            ...
            console.log(
              '[Checkout Success] Duplicate detected, finding existing voucher:',
